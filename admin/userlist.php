		...
		<tbody>
			...
			?>
				<tr>
					...
					<td>
						<input type="password" class="form-control password-input" id="epassword<?php echo $row['userid']; ?>" value="<?php echo $row['password']; ?>" readonly>
					</td>
					<td><img src="../<?php if (empty($row['photo'])) {
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var showPasswordButtons = document.querySelectorAll('.show-password');

    showPasswordButtons.forEach(function (button) {
      button.addEventListener('click', function () {
        var userId = this.getAttribute('data-userid');
        var passwordInput = document.getElementById('epassword' + userId);
